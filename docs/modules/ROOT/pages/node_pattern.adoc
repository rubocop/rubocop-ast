...
First, let's check what it looks like in the AST:
...
Instead, the following expression will match any number of arguments (and thus both examples above):

----
(send nil? :sum ...)
----

Note that `+...+` can appear anywhere in a sequence, for example `+(send nil? :sum ... int)+`
...
Using `+` would ensure that only sums with at least one argument would be matched.

----
(send nil? :sum int+)
----

The `?` can limit the match to only 0 or 1 nodes.
...
Let's make it a bit more complex and introduce floats:
...
Imagine you want to check if the number is odd and also positive:
...
case it is relative to that child (i.e. the current node). One can also use
...
Words ending with a `?` are treated as predicate methods.
They are called on the matched object, and the match succeeds if the method returns a truthy value.
Any method the matched object responds to can be used.

Example:

* `int_type?` can be used as a replacement for `(int _)`.

And refactoring the expression to allow both int or float types:

* `{int_type? float_type?}` can be used as a replacement for `({int float} _)`

You can also use predicates on child nodes:

* `(int odd?)` will match only odd integer literals, calling `odd?` on the
integer value.
...
Let's create an example where we're trying to find the symbols `user` and
...
Pay special attention to nil behavior:
...
* https://www.rubydoc.info/gems/rubocop-ast/RuboCop/AST/NodePattern[API documentation]
* https://github.com/rubocop/rubocop-ast/blob/master/lib/rubocop/ast/node_pattern.rb[Source code]
* https://nodepattern.herokuapp.com/[Node pattern debugger] — an online tool to try out patterns interactively
* https://github.com/rubocop/rubocop-ast/blob/master/spec/rubocop/ast/node_pattern_spec.rb[Specs] — useful for understanding each feature in detail
