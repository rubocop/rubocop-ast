...
`Rubocop::AST` provides a wrapper around `parser`'s `Node` class (in other words, `RuboCop::AST::Node.is_a?(Parser::AST::Node)`). In addition to a number of methods to make it easier to work with, the wrapper class also provides ways to inspect the *parents* of nodes, which the `parser` nodes do not support.
