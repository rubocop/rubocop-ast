...
|case_match|Pattern matching `case` statement (Ruby 2.7+).|First child is an expression node for the value being matched. Middle children are `in_pattern` nodes. Last child is the `else` branch expression, or `nil`.|case foo; in 1; bar; in 2; baz; else; qux; end|https://rubydoc.info/github/rubocop/rubocop-ast/RuboCop/AST/CaseMatchNode[CaseMatchNode]
...
|in_pattern|Pattern matching `in` clause (Ruby 2.7+). Must come inside a `case_match`.|Two children. First child is a pattern node. Last child is a body statement.|case foo; in [1, 2]; bar; end|https://rubydoc.info/github/rubocop/rubocop-ast/RuboCop/AST/InPatternNode[InPatternNode]

|index|Indexing operation (modern AST only, legacy mode emits `send` with `:[]`).|First child is the receiver node, remaining children are the index arguments.|foo[bar]|https://rubydoc.info/github/rubocop/rubocop-ast/RuboCop/AST/IndexNode[IndexNode]

|indexasgn|Index assignment (modern AST only, legacy mode emits `send` with `:[]=`).|First child is the receiver node, middle children are the index arguments, last child is the value being assigned.|foo[bar] = baz|https://rubydoc.info/github/rubocop/rubocop-ast/RuboCop/AST/IndexasgnNode[IndexasgnNode]

|int|Integer literal|One child, the integer value|-123|https://rubydoc.info/github/rubocop/rubocop-ast/RuboCop/AST/IntNode[IntNode]
...
|lambda|Lambda literal (modern AST only, legacy mode emits `send` with `:lambda`). Used as the first child of a `block` node.|None|->(foo) { bar }|https://rubydoc.info/github/rubocop/rubocop-ast/RuboCop/AST/LambdaNode[LambdaNode]
...
|module|Module definition|Two children. First child is a `const` node for the module name. Second child is a body statement.|module Foo; end|https://rubydoc.info/github/rubocop/rubocop-ast/RuboCop/AST/ModuleNode[ModuleNode]
...
|procarg0|Block argument wrapper. Wraps a single block argument when destructuring is possible. Must come inside an `args`.|One child, an `arg` node.|foo { \|bar\| baz }|https://rubydoc.info/github/rubocop/rubocop-ast/RuboCop/AST/Procarg0Node[Procarg0Node]
