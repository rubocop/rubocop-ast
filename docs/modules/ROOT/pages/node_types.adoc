...
|alias|Method alias|First child is a `sym`, `dsym` or `gvar` (new method name), second child is an expression node (old method name).|alias :foo :bar|https://rubydoc.info/github/rubocop-hq/rubocop-ast/RuboCop/AST/AliasNode[AliasNode]
